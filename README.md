...
**职位适配工作流（Tailor）：**
```
START → 分析JD → 匹配简历 → 生成定制简历 → END
```

**风格改写工作流（Rewrite）：**
```
START → 分析参考简历风格 → 生成风格差异 → 按风格改写各区块 → END
```

**特性：**
- 三种优化模式（诊断优化 / 职位适配 / 风格改写）
- SSE 实时推送节点输出
...
| t_interview_question | 面试题库 |
| t_interview_ai_analysis | AI 面试分析结果 |
| t_conversation | 面试对话 |
