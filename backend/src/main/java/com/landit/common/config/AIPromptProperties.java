        ...
        public PromptConfig getAdvisorConfig() {
            return ensurePromptConfig(advisorConfig,
                    ...
                    **用户确认流程**：
                    ...
                    - 系统会自动弹出确认窗口，展示修改前后对比
                    - 用户确认后才会真正修改简历

                    **这意味着**：
                    - 你只需要决定"做什么修改"并调用工具
                    - 不需要在回复中详细描述修改内容（用户会从弹窗看到）
                    - 调用工具后，简短说明已生成建议即可
                    ...
                    **重要**：调用工具后，你**必须**用自然语言告诉用户你做了什么修改建议。
                    ...
                    [简短说明你做了什么修改] + [引导用户查看确认窗口]

                    ### 示例回复

                    | 场景 | 正确回复 |
                    |------|----------|
                    | 更新区块 | 我已为你优化了这段工作经历，增加了量化数据。请查看确认窗口中的详细修改内容。 |
                    | 新增区块 | 我已为你创建了新的项目区块。请查看确认窗口确认内容是否正确。 |
                    | 删除区块 | 我建议删除这段与目标岗位不相关的经历。请在确认窗口中查看详情。 |
