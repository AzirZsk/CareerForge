...
| `useAudioTranscribe` | `composables/` | 音频转录（上传音频 -> 异步任务 -> 轮询结果） |
...
| 方法 | 路径 | 描述 |
|------|------|------|
| POST | /audio-transcribe | 创建音频转录任务 |
...
10. 新增异步任务功能需：
    ...
    - Composable 在 `composables/useAudioTranscribe.ts` 中扩展（轮询逻辑）

### 上下文文件
- **API 定义**：`backend/docs/openapi.yaml`
- **数据库结构**：`backend/src/main/resources/schema.sql`
- **技能定义**：`backend/src/main/resources/skills/`
- **前端类型**：`frontend/src/types/`（index.ts、ai-chat.ts、resume-optimize.ts、resume-tailor.ts、interview-center.ts、interview-voice.ts、job-position.ts、notification.ts）
...
- **异步任务 Controller**：`backend/src/main/java/com/landit/task/controller/TaskController.java`
- **异步任务 Service**：`backend/src/main/java/com/landit/task/service/AsyncTaskService.java`
- **音频转录 Service**：`backend/src/main/java/com/landit/task/service/AudioTranscribeTaskService.java`
- **前端任务类型**：`frontend/src/types/notification.ts`
...
| 区域 | 文件数 | 说明 |
|------|--------|------|
| 后端 Java 文件 | 247 | 全部子模块（common/user/resume/interview/statistics/job/chat/company/jobposition/voice/task） |
...
| 前端 Views | 13 | 页面组件（含 InterviewSession.vue、InterviewRecording.vue、InterviewDetail.vue） |
| 前端 Components | 88 | chat(10) + common(5) + resume(51) + interview-center(7) + interview/voice(4) + interview/recording(2) + App.vue |
| 前端 Composables | 20 | useAIChat, useConfirm, useFormValidation, useMarkdown, useResumeOptimize, useResumeTailor, useSectionDiff, useSectionEdit, useSectionHelper, useStageEdit, useStageTimer, useToast, useInterviewPreparation, useReviewAnalysis, useInterviewVoice, useStreamingAudio, useStreamAssist, useAudioRecorder, **useAudioTranscribe**, **useMicrophonePermission** |
| 前端 Utils | 2 | stageHelpers, recording-helpers |
| 前端 Types | 10 | index.ts, ai-chat.ts, resume-optimize.ts, resume-tailor.ts, interview-center.ts, interview-voice.ts, job-position.ts, notification.ts, marked.d.ts |
| 前端 API | 7 | user.ts, resume.ts, aiChat.ts, interview-center.ts, interview-voice.ts, job-position.ts, **task.ts** |
...
### Q: 如何使用异步任务功能（音频转录）？
A: 前端使用 `useAudioTranscribe` composable：
```typescript
import { useAudioTranscribe } from '@/composables/useAudioTranscribe'

const {
  // 状态
  isUploading, taskId, taskStatus, progress, result, error,
  // 方法
  uploadAndTranscribe, reset
} = useAudioTranscribe(interviewId)

// 上传音频并开始转录
await uploadAndTranscribe(audioFile)

// 轮询会自动进行，完成后 result 包含转录文本
```
