...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| 2026-04-21 | 2.9.0 | **AI 上下文全面扫描更新**：common/enums 新增 10 个枚举（BaseEnum/PositionStatus/RoundStatus/RoundType/InterviewSource/InterviewResult/PreparationSource/PreparationItemType/PreparationPriority/ReviewNoteType，11->21）；common 新增 JWT 认证基础设施（JwtProperties/JwtAuthenticationFilter/JwtUtil/SecurityUtils/WebMvcConfig）；前端新增 Font Awesome 图标库（plugins/fontawesome.ts）、position-status.ts 常量、request.ts Axios 封装；前端新增 DateTimePicker/UserDropdown/NotificationDropdown/DynamicListInput/StructuredItemRenderer 组件；前端 Components 93->101；前端新增 notification store、constants/plugins 目录 |
...
- **API 文档**：SpringDoc OpenAPI（访问 `/careerforge/swagger-ui.html`）
...
```bash
# 后端必需
export OPENAI_API_KEY=your_api_key                    # AI 模型 API Key
export ALIYUN_ACCESS_KEY_ID=your_access_key_id        # 阿里云语音服务
export ALIYUN_ACCESS_KEY_SECRET=your_access_key_secret

# 后端可选
export OPENAI_BASE_URL=https://api.openai.com         # OpenAI API 基础 URL
export AI_MODEL=gpt-4o                                # AI 模型选择
```
...
|                                                                                  |
|   问题预生成机制：                                                                 |
|   会话创建 --> QuestionPreGenerateService --> 批量生成 N 个问题 --> 缓存            |
|   (JD + 简历 --> LLM --> 问题列表 JSON --> 内存 + 数据库)                          |
|                                                                                  |
|   追问机制：                                                                       |
|   回答不符合预期 --> 实时调用 LLM --> 动态生成追问                                 |
|                                                                                  |
|   求助系统（SSE 流式）：                                                            |
...
| `VoiceWebSocketHandler` | `interview/voice/handler/` | WebSocket 消息处理与分发 |
...
| `RecordingMergeService` | `interview/voice/service/` | 录音合并服务 |
...
| `VoiceHandshakeInterceptor` | `interview/voice/interceptor/` | WebSocket 握手拦截器 |
...
| `useMicrophonePermission` | `composables/` | 麦克风权限管理（请求/检测/监听） |
...
- Composable：`useMicrophonePermission.ts` -- 麦克风权限请求、状态监听
...
| `PreparationContextBuilder` | `interview/graph/preparation/` | 准备工作流上下文构建 |
...
**注意**：`AnalyzeTranscriptNode` 虽然存在但实际由 `AnalyzeInterviewNode` 合并处理。
...
| `ReviewContextBuilder` | `interview/graph/review/` | 复盘工作流上下文构建 |
...
- 示例：`http://localhost:8080/careerforge/user/profile`
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | / | 获取任务列表（支持 type/status 筛选） |
| GET | /{taskId} | 获取任务详情 |
| DELETE | /{taskId} | 删除任务 |
| DELETE | /completed | 清理已完成的任务 |
...
10. 新增异步任务功能需：
    - 在 `task/` 下创建对应的 Service/DTO（任务类型、状态枚举）
    - Controller 在 `TaskController` 中定义 REST API
    - 前端类型定义在 `types/notification.ts` 中添加
    - 前端 API 在 `api/task.ts` 中添加
11. 新增认证功能需：
    - 在 `auth/` 下创建对应的 Service/Controller/DTO
    - 使用 JWT Token 进行身份验证
    - 前端在 `api/auth.ts` 中添加 API 调用
    - 前端在 `types/auth.ts` 中添加类型定义
    - 前端在路由守卫中检查登录状态
12. 新增枚举需：
    - 在 `common/enums/` 下创建枚举类，实现 `BaseEnum` 接口
...
- **图标库配置**：`frontend/src/plugins/fontawesome.ts`
- **HTTP 请求封装**：`frontend/src/utils/request.ts`
- **区块类型**：`backend/src/main/java/com/careerforge/common/enums/SectionType.java`
- **枚举基类**：`backend/src/main/java/com/careerforge/common/enums/BaseEnum.java`
- **Graph 公共常量**：`backend/src/main/java/com/careerforge/resume/graph/BaseGraphConstants.java`
- **优化工作流配置**：`backend/src/main/java/com/careerforge/resume/graph/optimize/ResumeOptimizeGraphConfig.java`
- **优化工作流常量**：`backend/src/main/java/com/careerforge/resume/graph/optimize/ResumeOptimizeGraphConstants.java`
- **面试准备工作流常量**：`backend/src/main/java/com/careerforge/interview/graph/preparation/InterviewPreparationGraphConstants.java`
- **复盘分析工作流常量**：`backend/src/main/java/com/careerforge/interview/graph/review/ReviewAnalysisGraphConstants.java`
- **面试中心 Handler**：`backend/src/main/java/com/careerforge/interview/handler/InterviewCenterHandler.java`
- **AI 提示词配置**：`backend/src/main/java/com/careerforge/common/config/AIPromptProperties.java`
- **问题预生成配置**：`backend/src/main/java/com/careerforge/interview/voice/service/QuestionPreGenerateService.java`
- **Schema 注册表**：`backend/src/main/java/com/careerforge/common/schema/SectionSchemaRegistry.java`
- **ChatAgent 配置**：`backend/src/main/java/com/careerforge/chat/config/ChatAgentConfig.java`
- **语音面试 Gateway**：`backend/src/main/java/com/careerforge/interview/voice/gateway/InterviewVoiceGateway.java`
- **阿里云语音配置**：`backend/src/main/java/com/careerforge/interview/voice/config/AliyunVoiceAutoConfiguration.java`
- **JWT 配置**：`backend/src/main/java/com/careerforge/common/config/JwtProperties.java`
- **JWT 过滤器**：`backend/src/main/java/com/careerforge/common/filter/JwtAuthenticationFilter.java`
- **安全工具**：`backend/src/main/java/com/careerforge/common/util/SecurityUtils.java`
- **认证 Controller**：`backend/src/main/java/com/careerforge/auth/controller/AuthController.java`
- **认证 Service**：`backend/src/main/java/com/careerforge/auth/service/AuthService.java`
- **前端认证 API**：`frontend/src/api/auth.ts`
- **前端认证类型**：`frontend/src/types/auth.ts`
- **前端职位状态常量**：`frontend/src/constants/position-status.ts`
...
| 后端 Graph 节点 | 15+1 | 优化 3 + 定制 3 + 改写 3 + 准备 3 + 复盘 3 + BaseGraphConstants（注：准备 5 节点含 CheckCompany/CheckJobPosition 条件路由节点） |
| Chat 工具 | 8 | GetResume/GetSection/UpdateSection/AddSection/DeleteSection/CreateResume/GetResumeList/SelectResume |
| 后端枚举 | 21 | BaseEnum + SectionType/Gender/ResumeStatus/ResumeType/InterviewType/InterviewStatus/QuestionDifficulty/ConversationRole/SuggestionType/ActivityType/ChangeType + PositionStatus/RoundStatus/RoundType/InterviewSource/InterviewResult/PreparationSource/PreparationItemType/PreparationPriority/ReviewNoteType |
...
| 前端 Components | 101 | chat(10) + common(8) + resume(51) + interview-center(25) + interview/voice(4) + interview/recording(2) + App.vue |
...
| 前端 Stores | 2 | index.ts（主 Store）、notification.ts（通知 Store） |
| 前端 Constants | 1 | position-status.ts |
| 前端 Plugins | 1 | fontawesome.ts |
...
3. 需要配置环境变量：
   ...
   - `ALIYUN_ACCESS_KEY_ID` - 阿里云语音服务
   - `ALIYUN_ACCESS_KEY_SECRET` - 阿里云语音服务
4. 新用户需先注册账号，登录后使用系统功能
...
### Q: 如何管理麦克风权限？
A: 前端使用 `useMicrophonePermission` composable：
```typescript
import { useMicrophonePermission } from '@/composables/useMicrophonePermission'

const {
  // 状态
  permissionStatus, isGranted, isDenied, isPrompt,
  // 方法
...
// 存储 token 到 localStorage，后续请求在 Header 中携带 Authorization: Bearer {token}
